...
我们提出 AutoResearcher，一个面向深度学习研究的自主多智能体框架。与现有 AI 科研助手不同，我们的系统覆盖了完整的实验生命周期：假设形成、代码实现、训练执行、结果验证和迭代优化。本框架引入七项关键创新，在七个版本的迭代中逐步增强：(1) **零成本监控** — 训练期间仅依赖进程检查和日志读取，LLM 调用成本为零；(2) **两层恒定大小记忆** — 有界记忆架构，上限约 5K 字符，防止长运行时的上下文溢出；(3) **最小工具集 Leader-Worker 架构** — 每个 Worker 仅配备 3-11 个工具，减少单次调用的 token 开销；(4) **深度模型架构分析** — 9 层 AST 分析覆盖数据流、信息瓶颈、梯度路径和结构完整性；(5) **Idea-架构对齐分析** — 自动将模型架构与 PROJECT_BRIEF 研究方案对比，检测核心创新是否被充分实现；(6) **实验信息价值估计 (VOI)** — 基于校准数据的假设成功率预测，避免低价值实验浪费 GPU 时间；(7) **帕累托前沿追踪** — 跨实验追踪方法×领域性能矩阵，避免重复次优方法。

在光场深度估计项目上部署验证中，系统自主完成了 14 个实验周期，将 Non-Lambertian MAE 从基线的 0.411 降至 0.103（75% 改进），Lambertian MAE 从 0.387 降至 0.165（57% 改进），整体 MAE 从 0.411 降至 0.126（69% 改进）。系统在持续运行中自主发现了分辨率-领域权衡、FFT 分支代表性不足、以及 Lambertian 硬平台等关键科学发现。
...
- **跨版本实验验证**：在光场深度估计项目上展示 8 个版本的持续改进（v1→v8）
...
系统由 12 个核心 Python 模块组成，共 13,307 行代码：

| 模块 | 行数 | 职责 |
|------|------|------|
| `core/loop.py` | 2,268 | 主循环：THINK/EXECUTE/MONITOR/REFLECT 四阶段协调 |
| `core/model_analyzer.py` | 2,282 | 模型架构分析 mixin：9 层 AST 分析、运行时探针、消融实验设计 |
| `core/verifier.py` | 1,924 | 实验验证器：9 层逆向工程验证（数据/训练/模型/结构） |
| `core/agents.py` | 1,356 | LLM 调用层：多 provider 故障转移、分层模型选择 |
| `core/tools.py` | 1,667 | 工具注册表：17 个工具定义 + 执行逻辑 + 安全验证 |
| `core/visual_analyzer.py` | 853 | 视觉分析模块：推理 + 多模态诊断（MCP + API） |
| `core/memory.py` | 831 | 两层记忆：恒定大小日志 + SQLite 实验历史 + Pareto 追踪 |
| `core/domain_knowledge.py` | 559 | 动态领域知识注入：方法假设检测、数据约束分析 |
| `core/mcp_client.py` | 724 | MCP 客户端 mixin：SSE + stdio 传输、服务检测、视觉工具 |
| `core/monitor.py` | 283 | 零成本监控：进程轮询、日志读取、GPU 状态 |
| `core/obsidian.py` | 304 | Obsidian 导出器：仪表板 + 每日笔记 |
| `gpu/` | 240 | GPU 检测 + 云保活工具 |
| `core/verifier.py` | 1,924 | VERIFY 阶段：10 层模块级验证 + 训练曲线分析 |
| `core/memory.py` | 820 | 持久化存储：SQLite 5 表 + 两层记忆管理 |
| `core/visual_analyzer.py` | 985 | 视觉分析：多模态 LLM 诊断预测结果 |
| `core/monitor.py` | 264 | 零成本训练监控：进程检查 + GPU 检查 + 日志尾部 |
| `core/obsidian.py` | 304 | Obsidian 同步：实时仪表盘 + 日记导出 |
...
| 指标 | 值 |
|------|-----|
| 总代码行数 | 13,307 (Python, 12 modules) |
| 核心模块数 | 8 |
| 工具总数 | 17 |
| 智能体类型 | 5 |
| SQLite 表数 | 5 |
...
| **代码总量** | 13,307 行 (12 modules) | ~3,000 行 (估) |
| **部署规模** | 1 项目, 14 周期 | 4 项目, 500+ 周期 |
| **最大改进** | 75% (Non-Lambertian MAE) | 52% |
| **开源地址** | 本地部署 | github.com/Xiangyue-Zhang/auto-deep-researcher-24x7 |
...
实验规划依赖 LLM 的推理能力，缺乏正式的探索策略（如贝叶斯优化）。集成结构化搜索方法可能改善样本效率。
...
我们提出了 AutoResearcher，一个面向深度学习研究的自主多智能体框架。通过八个版本的迭代演进（v1→v8），系统从基础的三阶段循环发展为包含深度模型分析、Idea-架构对齐、实验价值估计、帕累托前沿追踪、动态领域知识注入和研究方向守护的完整科研智能平台。
